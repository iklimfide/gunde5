-- Anasayfa arama terimleri: master ekle / düzenle / sil (hard delete)
-- SQL Editor'da bir kez Run.
-- Eski index_arama_terim_engelli kara listesini kaldırır.
...
drop table if exists public.index_arama_terim_engelli cascade;
...
begin
    ...
    set payload = jsonb_set(
        ...
        '{query}',
        to_jsonb(v_yeni::text),
        true
    )
    where e.event_type = 'index_search'
      and private.arama_terim_norm(e.payload->>'query') = v_eski;
...
begin
    ...
    delete from public.site_analytics_events e
    where e.event_type = 'index_search'
      and private.arama_terim_norm(e.payload->>'query') = v_terim;
    get diagnostics v_analytics_silinen = row_count;

    return jsonb_build_object('ok', true, 'terim', v_terim, 'analytics_silinen', v_analytics_silinen);
end;
$$;
...
begin
    ...
    foreach v_raw in array p_terimler loop
        ...
        where e.event_type = 'index_search'
          and private.arama_terim_norm(e.payload->>'query') = v_terim;
        get diagnostics v_parca = row_count;
        v_analytics_silinen := v_analytics_silinen + v_parca;
    ...
    return jsonb_build_object('ok', true, 'silinen', v_silinen, 'analytics_silinen', v_analytics_silinen);
end;
$$;
