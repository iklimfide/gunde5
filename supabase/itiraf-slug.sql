...
declare
    ...
    v_i int;
begin
    if p_words is null then return 'hikaye'; end if;

    foreach v_word in array p_words loop
        if v_word is null or v_word = '' then continue; end if;
        if public.itiraf_slug_kelime_gecerli(v_word, p_stop_filtre) then
            v_secilen := array_append(v_secilen, v_word);
            if coalesce(array_length(v_secilen, 1), 0) >= p_max_kelime then exit; end if;
        end if;
    end loop;
    ...
    if coalesce(array_length(v_secilen, 1), 0) = 0 then
        foreach v_word in array p_words loop
            ...
            if v_word ~ '^[a-z0-9]{2,}$' then
                ...
                if coalesce(array_length(v_fallback, 1), 0) >= least(3, p_max_kelime) then exit; end if;
            end if;
        end loop;
        v_secilen := v_fallback;
    end if;

    if coalesce(array_length(v_secilen, 1), 0) = 0 then return 'hikaye'; end if;
    ...
    for v_i in 1..coalesce(array_length(v_secilen, 1), 0) loop
        ...
        if length(v_deneme) > p_max_uzunluk then exit; end if;
    ...
    if length(v_secilen[1]) <= p_max_uzunluk then return v_secilen[1]; end if;
    return 'hikaye';
end;
$$;
...
begin
    ...
    if nullif(trim(coalesce(p_hint, '')), '') is not null then
        v_words := regexp_split_to_array(public.itiraf_slug_normalize(trim(p_hint)), '-');
    ...
    if coalesce(v_base, '') = '' then v_base := 'hikaye'; end if;
...
-- ---------------------------------------------------------------------------
-- 3) Trigger — yalnızca slug_hint değişince slug yenile
-- ---------------------------------------------------------------------------
